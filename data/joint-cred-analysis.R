...
key.data <- select(atop.milsup, latent.depth.mean, avg.democ, econagg.dum, uncond.milsup, 
...
theta.formula <- ~ num.mem + low.kap.sc + avg.democ + s(begyr)
eq.sigma <- ~ 1
...
theta.formula.max <- ~ num.mem + low.kap.sc + max.democ + s(begyr)
...
# examine the results: 
copulas[14] # Frank copula again minimizes AIC and has best convergence
joint.gjrm.max <- gjrm.models.max[[14]] 
